...
def run():
    ...
    try:
        ...
        logger.error(f"公告发现流程异常: {e}")
        logger.error(traceback.format_exc())
        notify_error(stage="公告发现流程", error=str(e), detail=traceback.format_exc())
...
def _do_discover():
    ...
    try:
        ...
        logger.error(f"巨潮公告搜索失败: {e}")
        notify_error(stage="巨潮公告搜索", error=str(e), detail=traceback.format_exc())
        announcements = []
    ...
    for ann in new_announcements:
        try:
            _process_one_offer(ann)
        except Exception as e:
            logger.error(f"处理要约公告异常: {e}")
            notify_error(
                stage="要约收购公告处理",
                error=str(e),
                detail=f"{ann.get('secName','')} - {ann.get('announcementTitle','')}\n{traceback.format_exc()}",
            )
        time.sleep(2)
...
def _process_one_offer(ann: dict):
    """处理单条要约收购公告"""
    ann_id = make_announcement_id(ann)
    ...
    for kw in keywords:
        ...
        for ann, ann_id in new_anns:
            ...
            try:
                ...
                logger.error(f"  处理公告失败 [{ann.get('secName','')}]: {e}")
                notify_error(
                    stage=f"额外公告处理({kw})",
                    error=str(e),
                    detail=f"{ann.get('secName','')} - {ann.get('announcementTitle','')}",
